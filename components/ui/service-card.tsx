    ...
    const CardContent = (
      ...
      >
        <div className="relative z-10 flex flex-col h-full gap-4">
          <h3 className="text-xl font-body font-bold tracking-tight text-white">{title}</h3>
          {children}
          {(href || onClick) && (
            <a
              href={href}
              ...
              className="mt-auto flex items-center text-xs font-semibold text-white/90 group-hover:text-white group-hover:underline"
            >
              EXPLORE
        ...
        {imgSrc && (
          <motion.img
            src={imgSrc}
            alt={imgAlt || title}
            className="absolute right-0 bottom-0 w-24 h-24 md:w-40 md:h-40 object-contain opacity-100 group-hover:opacity-100 translate-x-4 translate-y-2"
